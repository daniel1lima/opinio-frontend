...
import { Box, Typography, useTheme, Modal, TextField, Button, Slider, Switch, Stack, Alert, Tooltip, IconButton, CircularProgress } from "@mui/material";
...
const Integrations = () => {
  ...
  const [integrations, setIntegrations] = useState([
    { name: "Yelp", description: "Customer reviews and ratings.", icon: YelpIcon, status: false, fields: [{ label: "Business ID", value: "" }], last_sync: null },
    { name: "Google Reviews", description: "Manage your Google reviews.", icon: GoogleIcon, status: false, fields: [{ label: "Business ID", value: "" }, { label: "API Key", value: "" }], last_sync: null },
    { name: "Tripadvisor", description: "Travel and restaurant reviews.", icon: TripAdvisorIcon, status: false, fields: [{ label: "API Key", value: "" }], last_sync: null },
  ]);
  ...
  useEffect(() => {
    const eventSource = new EventSource(`${process.env.REACT_APP_SERVICES_BASE_URL}/job_status_webhook?company_id=${localStorage.getItem('company_id')}`);
  ...
  const fetchIntegrations = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/company_connections?company_id=${companyId}`);
      if (!response.ok) {
      ...
      const updatedIntegrations = integrations.map(integration => {
        const matchingConnection = data.connections.find(conn => conn.type.toLowerCase() === integration.name.toLowerCase());
        if (matchingConnection) {
          return {
            ...
            fields: integration.fields.map(field => ({
              ...field,
              value: matchingConnection.config[field.label.toLowerCase().replace(' ', '_')] || ''
            })),
  ...
  const getIntegrationStatus = (integration) => {
    if (jobStatus.connector_type === integration.name) {
      if (jobStatus.status === 'pending') {
        ...
        return differenceInSeconds(new Date(), syncDate) <= 259200 ? 'recent' : 'attention';
      }
    }
    return integration.last_sync ? (needsAttention(integration.last_sync) ? 'attention' : 'recent') : 'none';
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const addConnectionResponse = await fetch(`${baseUrl}/add_connection`, {
        ...
        body: JSON.stringify({ ...newIntegrations[selectedIntegration], company_id: companyId, last_sync: "" }),
      });
    ...
    return (
      <React.Fragment>
        ...
        <Tooltip title="Integration is syncing">
          <CircularProgress size={21} sx={{ mr: 2, 'svg circle': { stroke: 'url(#my_gradient)' } }} />
        </Tooltip>
      </React.Fragment>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Box display="flex" alignItems="center" mb={2}>
            <WarningAmberIcon sx={{ mr: 3, color: theme.palette.warning.main }} />
            <Typography id="confirmation-modal-title" variant="h5" component="h2" sx={{ fontWeight: 'bold', color: '#333' }}>
              Remove Integration
            </Typography>
          </Box>
